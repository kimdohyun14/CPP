...
struct _tagPlayer
{
	...
	int		iExp;
	int		iLevel;
	_tagItem	tEquip[EQ_MAX];  // 아이템 장착
	bool	    bEquip[EQ_MAX];  // 장착 여부 확인
...
_tagLevelUpStatus CreateLevelUpStatus(int iAttackMin, int iAttackMax,
...
int main()
{
	...
	g_tLvUpTable[JOB_KNIGHT - 1] = CreateLevelUpStatus(4, 10, 8, 16, 50, 100, 10, 20);
	g_tLvUpTable[JOB_ARCHER - 1] = CreateLevelUpStatus(10, 15, 5, 10, 30, 60, 30, 50);
	g_tLvUpTable[JOB_WIZARD - 1] = CreateLevelUpStatus(15, 20, 3, 7, 20, 40, 50, 100);
...
// 입력 오류일 경우 혹은 잘못된 번호를 입력한 경우 MM_NONE을 반환한다.
int OutputMainMenu()
{
	system("cls");
	cout << "**************************** 로비 ****************************" << endl;
...
// 숫자 입력을 받는다. 입력 오류까지 처리해주고
// INT_MAX가 리턴되면 입력 오류이다.
int InputInt()
{
	int iInput;
	cin >> iInput;
...
void OutputMonster(_tagMonster* pMonster)
{
	...
	cout << "획득경험치 : " << pMonster->iExp << "\t획득골드 : " << pMonster->iGoldMin <<
		" - " << pMonster->iGoldMax << endl << endl;
}
...
void Battle(_tagPlayer* pPlayer, _tagMonster* pMonster)
{
	...
	if (pMonster->iHP <= 0)
	{
		...
		int iGold = (rand() % (pMonster->iGoldMax - pMonster->iGoldMin) + pMonster->iGoldMin);
...
void SetPlayer(_tagPlayer* pPlayer)
{
	// 플레이어 이름을 입력받는다.
	cout << "이름 : ";
	cin.getline(pPlayer->strName, NAME_SIZE - 1); // 마지막에 NULL문자를 고려해서 -1을 해준다.
...
void RunStore(_tagInventory* pInventory, _tagItem* pWeapon, _tagItem* pArmor)
{
	while (true)
	{
		...
		{
		case SM_WEAPON:
			BuyItem(pInventory, pWeapon, STORE_WEAPON_MAX, IT_WEAPON);
			break;
		case SM_ARMOR:
			BuyItem(pInventory, pArmor, STORE_ARMOR_MAX, IT_ARMOR);
			break;
		case SM_BACK:
			return;
		}
...
void RunInventory(_tagPlayer* pPlayer)
{
	while (true)
	{
		...
		// 장착되어있지 않은 경우 인벤토리 아이템을 장착창으로 옮기고
		// 인벤토리는 1칸이 비워지게 된다.
		else
		{
			pPlayer->tEquip[eq] = pPlayer->tInventory.tItem[idx];
